...
class Knawat_Dropshipping_Woocommerce_Orders {
	...
	public function knawat_dropshipwc_create_type_order( $parent_order, $order_type, $order_items ) {
		...
		$order_data = apply_filters(
			...
			array(
				'post_type'     => 'shop_order',
				'post_title'    => sprintf( __( 'Order &ndash; %s', 'dropshipping-woocommerce' ), strftime( _x( '%1$b %2$d, %Y @ %I:%M %p', 'Order date parsed by strftime', 'dropshipping-woocommerce' ) ) ),
	...
	public function knawat_dropshipwc_get_order_item_meta_map() {
		return apply_filters(
			'knawat_dropshipwc_get_order_item_meta_keymap',
			array(
			...
			case 'kwd_suborder':
				...
				if ( $suborder_ids ) {
					foreach ( $suborder_ids as $suborder_id ) {
						...
						if ( version_compare( WC_VERSION, '3.3', '>=' ) ) {
							printf(
								'<div class="kwd_suborder_item"><strong><a href="%s">#%s</a></strong> <mark class="order-status status-%s"><span>%s</span></mark></div>',
								$order_uri,
							...
							printf(
								'<div class="kwd_suborder_item order_status column-order_status" style="display:block"><mark class="%s tips" data-tip="%s">%s</mark> <strong><a class="row-title" href="%s">#%s</a></strong></div>',
			...
			case 'knawat_dropshipwc-suborders':
				...
				foreach ( $suborder_ids as $suborder_id ) {
					...
					if ( version_compare( WC_VERSION, '3.3', '>=' ) ) {
						...
						printf(
							'<div class="kwd_suborder_item order_status column-order_status"><mark class="%s tips" data-tip="%s">%s</mark> <strong><a class="row-title" href="%s">#%s</a></strong></div>',
							esc_attr( sanitize_html_class( $suborder->get_status() ) ),
